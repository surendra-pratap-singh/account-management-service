...
public class ErrorHandler {
	...
	private Object insufficientFundsException(Exception ex) {
		return new Response(HttpStatus.UNPROCESSABLE_ENTITY, Optional.ofNullable(ex.getMessage()));
	}

	@ExceptionHandler(InternalServerErrorException.class)
	@ResponseBody
	private Object internalServerErrorException(Exception ex) {
		return new Response(HttpStatus.INTERNAL_SERVER_ERROR, Optional.ofNullable(ex.getMessage()));
	}
	...
	private Object returnInvalidRequest(Exception ex) {
		return new Response(HttpStatus.BAD_REQUEST, Optional.ofNullable(ex.getMessage()));
	}
